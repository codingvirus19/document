      ...
      <div className={styles.modalContainer} onClick={this.props.onClickFalse}>
        <div className={styles.modalContainer__inner} onClick={(e) => e.stopPropagation()}>
          <header className={styles.inner__header}>
            {this.props.contents != null ?
              <span className={styles.header__title}>
                <h2 className={styles.title__h2}>{this.props.contents}</h2>
                </span>
              : null}

            <span onClick={this.props.onClickFalse}className={styles.header__close}>
              <button className={styles.close__icon} >&times;</button>
              </span>
          </header>
          {this.props.contents != null && this.props.contents != "메모삭제안됨" ?
            <div className={styles.contents}>
              <p className={styles.contents__p}>{this.props.contents}</p>
              <p>를 계속 진행하시겠습니까?</p>
            </div> : null}
          ...
          <footer className={styles.footer}>
            ...
            {this.props.contents == "그룹 삭제" ?
              <button onClick={this.onAjaxOutGroup.bind(this)} className={styles.confirm_btn}>확인</button> : null}

            {/* GroupOutAlone의 버튼 클릭시 아래 함수 실행 */}
            {this.props.contents == "그룹 나가기" ?
              <button onClick={this.onAjaxOutGroupAlone.bind(this)} className={styles.confirm_btn}>확인</button> : null}

            {/* onClickDelete를 실행 시 기존의 modal이 꺼지고 새로운  */}
            {this.props.contents == "메모삭제" ?
              <button onClick={this.props.onClickDelete} className={styles.confirm_btn}>확인</button> : null}
